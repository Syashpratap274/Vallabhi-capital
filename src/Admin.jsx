...
function Homepage({ data, update, setModal }) {
  const h = data.homepage || {}; const items = sortItems(data.products?.items || []); const inds = sortItems(data.industries?.items || []);
  const moveProduct = (id, direction) => update((d) => { const arr = sortItems(d.products.items || []); const productIds = arr.map((p) => p.id); const i = productIds.indexOf(id); const j = direction === "up" ? i - 1 : i + 1; if (i < 0 || j < 0 || j >= productIds.length) return; [productIds[i], productIds[j]] = [productIds[j], productIds[i]]; productIds.forEach((pid, n) => { const p = d.products.items.find((x) => x.id === pid); if (p) p.order = n + 1; }); }, "Homepage product order updated");
  return <>
    ...
    <section className="adm-panel"><div className="adm-panel-title"><div><h2>Homepage Product Order</h2><p className="adm-note">All published products appear on the homepage. Use the arrows to choose their display order.</p></div></div>{items.map((p, i) => <div className="adm-item-row" key={p.id}><div>{(p.productLogo || p.cardImage) && <img src={p.productLogo || p.cardImage} alt="" />}<div><strong>{p.name || "Unnamed product"}</strong><small>Homepage position {i + 1} · {p.shortDescription || "No short description"}</small></div></div><div className="adm-actions"><button onClick={() => moveProduct(p.id, "up")} disabled={i === 0}>↑</button><button onClick={() => moveProduct(p.id, "down")} disabled={i === items.length - 1}>↓</button></div></div>)}</section>
